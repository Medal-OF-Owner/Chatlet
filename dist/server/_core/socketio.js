...
import { addMessage, reserveNickname, releaseNickname, checkNicknameAvailable } from "../db";
const users = new Map();
export function setupSocketIO(httpServer) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("join_room", async (data) => {
            ...
            const existingUsers = Array.from(users.values()).filter((u) => u.roomId === roomId && u.id !== socket.id);
