...
import { users, rooms, messages, activeNicknames, accounts } from "../drizzle/schema";
...
export async function getOrCreateRoom(slug) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
...
export async function addMessage(roomId, nickname, content, fontFamily, profileImage) {
    const db = await getDb();
    if (!db)
        return;
...
export async function reserveNickname(nickname) {
    const db = await getDb();
    if (!db)
        return false;
    try {
...
export async function releaseNickname(nickname) {
    const db = await getDb();
    if (!db)
        return;
    await db.delete(activeNicknames).where(eq(activeNicknames.nickname, nickname));
}
export async function createAccount(email, nickname, password) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    try {
